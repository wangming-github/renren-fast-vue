  ...
  <div class="mod-config">
    ...
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle"
      ...
      <el-table-column prop="showStatus" header-align="center" align="center" label="显示状态">
        <template slot-scope="scope">
          <!-- 将整行信息传递给 updateBeandShowStatus()-->
          <!-- 激活active-value="1" 没激活inactive-value="0"  ":active-value="1" 加冒号为数字类型 :inactive-value="0"加冒号为数字类型" -->
          <el-switch v-model="scope.row.showStatus" @change="updateBeandShowStatus(scope.row)" active-color="#13ce66"
...
export default {
  ...
  methods: {
    updateBeandShowStatus(status) {
      console.log("整行数据的最新信息:", status);
      // 结构获取对象指定属性值
